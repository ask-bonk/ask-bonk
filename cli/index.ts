...
type EventTrigger = 'issue_comment' | 'pull_request_review_comment' | 'issues' | 'pull_request' | 'schedule' | 'workflow_dispatch'
...
async function runInstall() {
	...
	if (setSecretConfirm) {
		...
		p.log.info(`Set ${providerConfig.keyName} manually at: https://github.com/${targetRepo}/settings/secrets/actions`)
	}
...
async function runWorkflow(repo?: string, providerConfig?: ProviderConfig, canWriteRemote: boolean = true) {
	...
	while (addAnother) {
		const preset = (await p.select({
			...
			options: [
				{ value: 'bonk', label: 'bonk', hint: 'mention-triggered interactive assistant' },
				{ value: 'scheduled', label: 'scheduled', hint: 'run on a schedule (e.g. weekly updates)' },
				{ value: 'triage', label: 'triage', hint: 'run on new issues' },
				{ value: 'review', label: 'review', hint: 'on-demand PR review via /review' },
				{ value: 'custom', label: 'make my own', hint: 'custom event triggers' },
			],
		})) as WorkflowPreset
...
async function buildPresetWorkflow(preset: WorkflowPreset, providerConfig?: ProviderConfig): Promise<WorkflowConfig> {
...
async function buildCustomWorkflow(providerConfig?: ProviderConfig): Promise<WorkflowConfig> {
	...
	const events = (await p.multiselect({
		...
		options: [
			{ value: 'issue_comment', label: 'Issue comment', hint: 'on new issue comments' },
			{ value: 'pull_request_review_comment', label: 'PR review comment', hint: 'on PR review comments' },
			...
			{ value: 'workflow_dispatch', label: 'Manual trigger', hint: 'allow manual runs' },
		],
		required: true,
	})) as EventTrigger[]
	...
	const hasCommentTriggers = events.includes('issue_comment') || events.includes('pull_request_review_comment')
	...
	const promptSource = (await p.select({
		...
		options: [
			{ value: 'comment', label: 'Comment body', hint: 'use the triggering comment as the prompt' },
			{ value: 'issue', label: 'Issue body', hint: 'use the issue body as the prompt' },
			{ value: 'custom', label: 'Custom prompt', hint: 'provide a custom prompt' },
		],
	...
	const permissions = (await p.select({
		message: 'Permissions',
		options: [
			{ value: 'write', label: 'Write', hint: 'can create commits, branches, PRs' },
